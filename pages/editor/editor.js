...
function setThicc() {
    document.querySelectorAll("ion-range").forEach(E => {
        if (E.id == "thickness") {
            val = Number(E.value)
            if (val && val != 0) {
                lineT = val ** 2
...
function setRed() {
    document.querySelectorAll("ion-range").forEach(E => { if (E.id == "red") { red = Number(E.value) } })
}
function setGreen() {
    document.querySelectorAll("ion-range").forEach(E => { if (E.id == "green") { green = Number(E.value) } })
}
function setBlue() {
    document.querySelectorAll("ion-range").forEach(E => { if (E.id == "blue") { blue = Number(E.value) } })
}


function setMagic() {
    //document.querySelectorAll("ion-range").forEach(E => {if (E.id == "how") {  magicNum = Number(E.value)}})
    document.querySelectorAll("ion-input").forEach(E => { if (E.id == "magicitemnumber") { magicNum = Number(E.value) } })

}
function setWiggles() {
    document.querySelectorAll("ion-range").forEach(E => { if (E.id == "wiggle") { wigglenum = Number(E.value) } })
}
...
function buttonControl(E, newmode) {
    if (mode != newmode) {
        ...
        E.setAttribute("color", "secondary")
        drawbutton.setAttribute("color", "tertiary")
        activebutton = drawbutton
        mode = "draw"
    }
    ham(DIVsvg)
...
function pauser(E) {
    ...
    if (plause) {
        ...
        E.setAttribute("color", "danger")
        E.innerHTML = `<ion-label>
        <ion-icon src="/ionicon:pause-circle">
        </ion-icon>
      </ion-label>`
    }
    getCSVG("pauser")
